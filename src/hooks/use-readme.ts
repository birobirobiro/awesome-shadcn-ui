...
/**
 * Extracts a normalized "owner/repo" key from a URL, or null if it doesn't
 * point to a GitHub repository. Mirrors scripts/update-stars.js so star
 * lookups match the keys in public/stars.json.
 */
export function githubRepoFromUrl(rawUrl: string): string | null {
  let url: URL;
  try {
    url = new URL(rawUrl);
  } catch {
...
/**
 * Fetches the star counts generated by scripts/update-stars.js (committed
 * weekly to public/stars.json by a GitHub Action). Returns an empty map on
 * any failure — stars are a progressive enhancement, never a blocker.
 */
...
export async function fetchAndParseReadme(): Promise<Resource[]> {
  ...
  try {
    ...
    for (const line of lines) {
      ...
      const repoKey = githubRepoFromUrl(parsed.url);
      const stars = repoKey != null ? starCounts[repoKey] : undefined;
      resources.push({ id, order: resources.length, stars, ...parsed });
    }
